...
{{-- Downloader connection status --}}
<div class="nx-card mt-6 flex flex-wrap items-center justify-between gap-3 p-5 {{ $agent['connected'] ? 'border-success/25' : 'border-[#ff6b81]/25' }}">
    <div class="flex items-center gap-3">
        ...
        <div>
            <div class="text-sm font-semibold">{{ $agent['connected'] ? 'เชื่อมต่อ Hive Download แล้ว — พร้อมดาวน์โหลด' : 'ยังไม่ได้เชื่อมต่อ Hive Download' }}</div>
            <div class="text-xs text-cream/45">
                @if ($agent['last_seen'])เห็นล่าสุด {{ $agent['last_seen']->diffForHumans() }}@else ยังไม่เคยเชื่อมต่อ — เปิด NetwixSync บนเครื่องบ้าน @endif
            </div>
        </div>
    </div>
    @unless ($agent['connected'])
        <span class="rounded-lg bg-[#ff6b81]/10 px-3 py-2 text-xs text-[#ff6b81]">ปุ่มนำเข้าจะทำงานเมื่อเชื่อมต่อแล้ว · รัน <code class="rounded bg-white/10 px-1">NetwixSync --loop</code></span>
    @endunless
</div>
            ...
            <tbody>
                @forelse ($titles as $t)
                    <tr class="border-b border-white/[0.04] hover:bg-white/[0.02]">
                        <td class="px-5 py-3">
                            <div class="flex items-center gap-3">
                                <div class="relative h-12 w-[46px] flex-shrink-0 overflow-hidden rounded {{ $t->poster_url ? 'cursor-zoom-in' : '' }}"
                                     style="background:{{ $t->gradient }}" @if ($t->poster_url) data-zoom-src="{{ $t->poster_url }}" @endif>
                                    @if ($t->poster_url)
